...
IF NOT EXIST "%cd%\serverstarter-2.1.1.jar" (
	...
	%SYSTEMROOT%\SYSTEM32\bitsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground https://github.com/red2blue/ServerStarter/releases/download/v2.1.1/serverstarter-2.1.1.jar "%cd%\serverstarter-2.1.1.jar"
   GOTO MAIN
) ELSE (
   GOTO MAIN
)
